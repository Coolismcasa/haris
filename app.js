/* ═══════════════════════════════════════════════════════════
   ...
   Firebase Auth + Firestore + Products + Cart + Admin
   ═══════════════════════════════════════════════════════════ */
...
const FALLBACK_PRODUCTS = [
  { id:'p1', name:'Classic Red Tee', cat:'shirts', price:2490, oldPrice:3200, tag:'New',
    ...
    sizes:['S','M','L','XL','XXL'],
    colors:[{name:'Red',hex:'#E33A3A'},{name:'Navy',hex:'#0B1A30'}],
    fabric:'100% Combed Cotton', care:'Machine wash cold', sku:'CLM-SH-001',
    inStock:true, stock:50, lowStock:5 },
  { id:'p2', name:'Royal Oxford Shirt', cat:'shirts', price:4990, oldPrice:null, tag:null,
    ...
    fabric:'Egyptian Cotton Oxford', care:'Machine wash warm', sku:'CLM-SH-002',
    inStock:true, stock:30, lowStock:5 },
  { id:'p3', name:'Pleated Wide-Leg Trousers', cat:'pants', price:5890, oldPrice:6990, tag:'Bestseller',
    ...
    fabric:'Poly-wool blend', care:'Dry clean only', sku:'CLM-PT-001',
    inStock:true, stock:25, lowStock:5 },
  { id:'p4', name:'Relaxed Linen Pants', cat:'pants', price:4290, oldPrice:null, tag:null,
    ...
    fabric:'100% European Linen', care:'Machine wash cold', sku:'CLM-PT-002',
    inStock:true, stock:40, lowStock:5 },
  { id:'p5', name:'Moto Leather Jacket', cat:'jackets', price:18900, oldPrice:22500, tag:'Limited',
    ...
    fabric:'Full-grain leather', care:'Wipe clean', sku:'CLM-JK-001',
    inStock:true, stock:8, lowStock:3 },
  { id:'p6', name:'Navy Bomber Jacket', cat:'jackets', price:11900, oldPrice:null, tag:null,
    ...
    fabric:'Recycled nylon shell', care:'Machine wash cold', sku:'CLM-JK-002',
    inStock:true, stock:20, lowStock:5 },
  { id:'p7', name:'Oversized Navy Hoodie', cat:'hoodies', price:6490, oldPrice:7990, tag:'Bestseller',
    ...
    fabric:'85% Cotton / 15% Poly', care:'Machine wash cold', sku:'CLM-HD-001',
    inStock:true, stock:60, lowStock:5 },
  { id:'p8', name:'Fleece-Lined Zip Hoodie', cat:'hoodies', price:7290, oldPrice:null, tag:'New',
    ...
    fabric:'Cotton-poly blend', care:'Machine wash cold', sku:'CLM-HD-002',
...
/* ═══════ THEME (DARK DEFAULT) ═══════ */
(function initTheme() {
  const saved = localStorage.getItem('coolism_theme') || 'dark';
...
document.addEventListener('click', e => {
  if (e.target.closest('#themeToggle')) {
    ...
    localStorage.setItem('coolism_theme', next);
    toast(next === 'dark' ? '🌙 Dark royal blue' : '☀️ Off white');
  }
});
...
/* ═══════ AUTH STATE (SHOW PROFILE ICON WHEN LOGGED IN) ═══════ */
const authReady = new Promise(resolve => {
  auth.onAuthStateChanged(user => {
    ...
    if (user) {
      // Logged in: hide login button, show profile icon
      ...
      // Logged out: show login button, hide profile icon
...
function renderCategoryTiles() {
  ...
  grid.innerHTML = Object.keys(CATEGORIES).map(k => {
    ...
    return `<a href="#shop" class="cat-tile" data-filter="${k}">
  ...
  grid.querySelectorAll('.cat-tile').forEach(tile => {
    tile.addEventListener('click', () => {
      const chip = document.querySelector(`.chip[data-filter="${tile.dataset.filter}"]`);
      if (chip) chip.click();
...
function renderFooterCats() {
  ...
  if (!ul) return;
  ul.innerHTML = Object.keys(CATEGORIES).map(k =>
    `<li><a href="#shop" data-filter="${k}">${CATEGORIES[k].label}</a></li>`).join('');
  ul.querySelectorAll('a').forEach(a => a.addEventListener('click', () => {
    const chip = document.querySelector(`.chip[data-filter="${a.dataset.filter}"]`);
    if (chip) chip.click();
  }));
}
function renderFilters() {
  ...
  el.innerHTML = `<button class="chip active" data-filter="all">All</button>` +
    Object.keys(CATEGORIES).map(k =>
      `<button class="chip" data-filter="${k}">${CATEGORIES[k].label}</button>`).join('');
}
...
function renderCart() {
  ...
  if (!cart.length) {
    body.innerHTML = `<div class="cart-empty">
      ...
      <a href="#shop" class="btn btn-line" style="margin-top:16px">Start Shopping</a>
    </div>`;
    return;
  }
...
function openDetail(id) {
  ...
  document.getElementById('detailPrice').innerHTML =
    `<span class="now">${money(p.price)}</span>${p.oldPrice ? `<span class="was">${money(p.oldPrice)}</span>` : ''}`;
  set('detailDesc', p.desc || '');
  set('detailFabric', p.fabric || '—');
  set('detailCare', p.care || '—');
  set('detailSku', p.sku || '—');

  document.getElementById('detailSizes').innerHTML = p.sizes.map(s =>
    `<button class="opt-btn ${s === currentDetail.size ? 'active' : ''}" data-size="${s}">${s}</button>`).join('');
  document.getElementById('detailColors').innerHTML = p.colors.map(c =>
    `<button class="color-btn ${c.name === currentDetail.color ? 'active' : ''}" data-color="${c.name}"><span class="swatch" style="background:${c.hex}"></span><span class="color-name">${c.name}</span></button>`).join('');
  ...
  if (sw) {
    if (p.stock !== undefined && p.stock <= 5 && p.stock > 0) {
      sw.textContent = `Only ${p.stock} left in stock!`;
      sw.style.display = 'block';
    } else if (p.stock === 0) {
      sw.textContent = 'Out of stock';
      sw.style.display = 'block';
    } else {
...
function showImageIndex(idx) {
  ...
  document.getElementById('imgCounter').textContent = `${currentDetail.index + 1} / ${total}`;
    ...
    db.collection('users').doc(u.uid).get().then(snap => {
      if (snap.exists) {
        ...
        if (box) {
          ...
          if (pts >= 100) {
            box.hidden = false;
            document.getElementById('loyaltyAvail').textContent = pts;
            document.getElementById('loyaltyValue').textContent = money(calcLoyaltyDiscount(pts));
            window._userLoyaltyPoints = pts;
          } else {
            box.hidden = true;
            window._userLoyaltyPoints = pts;
          }
...
function updateCheckoutSummary() {
  ...
  if (useLoyalty && useLoyalty.checked) {
    const pts = window._userLoyaltyPoints || 0;
    loyaltyDisc = Math.min(calcLoyaltyDiscount(pts), subtotal);
  }
  const total = Math.max(0, subtotal + shipping - loyaltyDisc);
...
async function placeOrder(user, formData) {
  ...
  const useLoyalty = document.getElementById('useLoyalty');
  let loyaltyUsed = 0, loyaltyDisc = 0;
  if (useLoyalty && useLoyalty.checked) {
    const pts = window._userLoyaltyPoints || 0;
    loyaltyDisc = Math.min(calcLoyaltyDiscount(pts), subtotal);
    loyaltyUsed = loyaltyDisc * 10;
  }
  const total = Math.max(0, subtotal + shipping - loyaltyDisc);
  ...
  const order = {
    ...
    subtotal, shipping, loyaltyDiscount: loyaltyDisc, loyaltyUsed, total,
  ...
  try {
    ...
    const curPts = window._userLoyaltyPoints || 0;
    await db.collection('users').doc(user.uid).set({
      ...
      city: formData.city, district: formData.district, province: formData.province,
      loyaltyPoints: Math.max(0, curPts - loyaltyUsed) + earned
    }, { merge: true });
    ...
    const le = document.getElementById('loyaltyEarned');
    if (le && earned > 0) {
      le.hidden = false;
      document.getElementById('earnedPoints').textContent = earned;
    }
    document.getElementById('successModal').classList.add('show');
    setTimeout(() => document.getElementById('successModal').classList.remove('show'), 6000);
...
async function loadProfile(user) {
  ...
  try {
    ...
    const pts = d.loyaltyPoints || 0;
    const lp = document.getElementById('loyaltyPoints'); if (lp) lp.textContent = pts;
    const lvt = document.getElementById('loyaltyValueText');
    if (lvt) lvt.innerHTML = `Your points are worth <b>${money(calcLoyaltyDiscount(pts))}</b> in discount.`;
...
function renderProfileOrders(orders) {
  ...
  if (!orders.length) {
    c.innerHTML = `<div class="empty-state">
      ...
      <a href="index.html#shop" class="btn btn-silver">Start Shopping</a>
    </div>`;
    return;
  }
    ...
    return `<div class="order-card">
      <div class="order-card-head">
        <div>
          ...
          <p class="oc-date">${date} · ${o.itemCount||0} item(s)</p>
        </div>
      ...
      <div class="order-card-actions">
        <button class="link-btn" data-view-order="${o.id}">View Details →</button>
      </div>
    </div>`;
  }).join('');
...
function showOrderDetail(id, orders) {
  ...
  document.getElementById('odBody').innerHTML = `
    ...
    <div class="order-detail-block">
      ...
      ${o.loyaltyDiscount ? `<div class="row"><b>Loyalty</b><span>−${money(o.loyaltyDiscount)}</span></div>` : ''}
...
async function loadProducts() {
  ...
  if (!tbody) return;
  tbody.innerHTML = `<tr><td colspan="8" class="table-empty">Loading products…</td></tr>`;
...
function renderProductsTable(list) {
  ...
  if (!tbody) return;
  if (!list.length) {
    tbody.innerHTML = `<tr><td colspan="8" class="table-empty">No products yet. Click "+ Add Product".</td></tr>`;
    return;
  }
  ...
  if (newCat) {
    ...
    try {
      await db.collection('categories').doc(slug).set({
        label: newCat, sub: 'New collection', img: images[0] || '',
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      }, { merge: true });
      CATEGORIES[slug] = { label: newCat, sub: 'New collection', img: images[0] || '' };
    } catch (e) {}
  }
  if (!finalCat) return toast('Select or create a category');

  const data = {
    name, cat: finalCat, price, oldPrice, tag, images, sizes, sku,
...
function renderUsersTable(list) {
  ...
  if (!tbody) return;
  if (!list.length) {
    tbody.innerHTML = `<tr><td colspan="9" class="table-empty">No users yet.</td></tr>`;
    return;
  }
...
function showAdminUser(uid) {
  ...
  document.getElementById('auBody').innerHTML = `
    <div class="order-detail-block">
      ...
      <div class="row"><b>Loyalty Points</b><span><b>${u.loyaltyPoints||0}</b></span></div>
...
document.addEventListener('click', e => {
  ...
  if (sb) { updateOrderStatus(sb.dataset.status); return; }
  ...
  if (adminTab) {
    ...
    ['products','orders','users','analytics'].forEach(p => {
...
document.addEventListener('input', e => {
  ...
  /* Image preview as you type filename */
  ...
  /* Checkout: recalculate when address changes */
...
document.addEventListener('change', e => {
  if (e.target.id === 'useLoyalty') updateCheckoutSummary();
});
...
if (document.getElementById('exportOrders')) document.getElementById('exportOrders').addEventListener('click', () => {
  const h = ['Order ID','Date','Customer','Email','Phone','Address','City','District','Province','Items','Subtotal','Shipping','Loyalty Disc','Total','Status'];
  const r = allOrders.map(o => [
    ...
    o.itemCount||0, o.subtotal||0, o.shipping||0, o.loyaltyDiscount||0, o.total||0, o.status||'pending'
  ]);
...
if (document.getElementById('exportUsers')) document.getElementById('exportUsers').addEventListener('click', () => {
  const h = ['UID','Email','Name','Phone','City','District','Province','Provider','Verified','Loyalty Points','Joined'];
  const r = allUsers.map(u => [
    u.uid||u.id, u.email||'', `${u.firstName||''} ${u.lastName||''}`.trim(),
    u.phone||'', u.city||'', u.district||'', u.province||'',
    u.provider||'', u.emailVerified?'Yes':'No', u.loyaltyPoints||0,
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') {
    ...
    ['detailModal','productModal','sizeGuideModal','orderDetailModal','adminOrderModal','adminUserModal','editModal','successModal']
